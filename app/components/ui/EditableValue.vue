...
<template>
  <!-- Editing Mode -->
  <div v-if="isEditing" class="inline-flex items-center gap-1 bg-zinc-950 p-1 rounded-lg border border-emerald-500/80 shadow-lg z-30" @click.stop>
    <input
      ref="inputRef"
      v-model.number="tempValue"
      type="number"
      :step="step"
      class="w-24 bg-zinc-900 border border-zinc-700 px-2 py-0.5 text-center font-mono font-bold text-white text-xs rounded focus:border-emerald-400 focus:outline-none"
    ...
    <button
      type="button"
      class="flex h-5 w-5 items-center justify-center rounded bg-emerald-600/30 text-emerald-300 hover:bg-emerald-600 hover:text-white transition"
    ...
    <button
      type="button"
      class="flex h-5 w-5 items-center justify-center rounded bg-rose-600/30 text-rose-300 hover:bg-rose-600 hover:text-white transition"
      title="Annuler (Echap)"
  ...
  <span
    v-else
    class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 font-mono cursor-pointer hover:bg-zinc-800/80 hover:text-white transition border border-dashed border-transparent hover:border-zinc-700 select-none"
    :class="textColorClass"
    title="Cliquer pour modifier la valeur à la main"
    @click.stop="startEditing"
  >
    <span>{{ formattedDisplay }}</span>
    <svg class="h-3 w-3 text-zinc-500 opacity-0 group-hover:opacity-100 hover:opacity-100 transition" viewBox="0 0 20 20" fill="currentColor">
